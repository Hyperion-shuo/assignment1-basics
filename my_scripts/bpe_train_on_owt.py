...
# run with
# scalene run --outfile result.html my_scripts/bpe_train_on_owt.py
# uv run my_scripts/bpe_train_on_owt.py
if __name__ == "__main__":
    ...
    with open("data/owt_train_bpe_vocab.json", "w", encoding="utf-8") as f:
    ...
    with open("data/owt_train_bpe_merges.json", "w", encoding="utf-8") as f:
        merges_bytes = [[list(merge[0]), list(merge[1])] for merge in merges]
        json.dump(merges_bytes, f, indent=2)
    
    print(f"Saved vocab ({len(vocab)} tokens) to data/owt_train_bpe_vocab.json")
    print(f"Saved merges ({len(merges)} merges) to data/owt_train_bpe_merges.json")